...
DATA_PATH =  os.path.join(os.path.dirname(settings.BASE_DIR), 'data/C11.csv')
...
def best_fit_distribution(data, bins=200, ax=None):
    ...
    DISTRIBUTIONS = [
        ...
        st.foldcauchy,st.foldnorm,st.frechet_r,st.frechet_l,st.genlogistic,
        ...
        st.gumbel_l,st.halfcauchy,st.halflogistic,st.halfnorm,st.halfgennorm,
...
def get_best_distribution(data=None):
    data = pd.read_csv(DATA_PATH, sep=',', encoding='utf-8', header=0)['dbn_nez']

    plt.figure(figsize=(12,8))
    ax = data.plot(kind='hist', density=True, bins=50, alpha=0.5, color=plt.rcParams['axes.prop_cycle'].by_key()['color'][1])
...
def get_as(data=None):
    data = pd.read_csv(DATA_PATH, sep=',', encoding='utf-8', header=0)['dbe_nez']
